...
else if($_REQUEST['action'] == 'searchcustomer')
{
			...
			global $wpdb;
			$disp1 = $wpdb -> get_results("SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients . " WHERE " . $query . " order by name ASC");
			$count = count($disp1);
			if($count!=0)
			{
					?>
			...
			<?php
			for ($i = 0; $i < count($disp1); $i++)
			{
					$st = ($i%2 == 0)? '': 'background-color: #ffffff';
					?>
					<input type="hidden" id="id_<?php echo $i;?>" value="<?php echo $disp1[$i] -> id;?>"/>
					<input type="hidden" id="name_<?php echo $i;?>" value="<?php echo $disp1[$i] -> name;?>"/>
					<input type="hidden" id="lname_<?php echo $i;?>" value="<?php echo $disp1[$i] -> lastname;?>"/>
					<input type="hidden" id="email_<?php echo $i;?>" value="<?php echo $disp1[$i] -> email;?>"/>
					<input type="hidden" id="mobile_<?php echo $i;?>" value="<?php echo $disp1[$i] -> mobile;?>"/>
					<input type="hidden" id="telephone_<?php echo $i;?>" value="<?php echo $disp1[$i] -> telephone;?>"/>
					<input type="hidden" id="addressLine1_<?php echo $i;?>" value="<?php echo $disp1[$i] -> addressLine1;?>"/>
					<input type="hidden" id="addressLine2_<?php echo $i;?>" value="<?php echo $disp1[$i] -> addressLine2;?>"/>
					<input type="hidden" id="city_<?php echo $i;?>" value="<?php echo $disp1[$i] -> city;?>"/>
					<input type="hidden" id="postalcode_<?php echo $i;?>" value="<?php echo $disp1[$i] -> postalcode;?>"/>
					<input type="hidden" id="country_<?php echo $i;?>" value="<?php echo $disp1[$i] -> country;?>"/>
					<input type="hidden" id="comments_<?php echo $i;?>" value="<?php echo $disp1[$i] -> comments;?>"/>
					<tr style="<?php echo $st; ?> ">
					<td style="word-wrap: break-word"><?php echo $disp1[$i] -> name;?></td>
					<td style="word-wrap: break-word"><?php echo $disp1[$i] -> lastname;?></td>
					<td style="word-wrap: break-word"><?php echo $disp1[$i] -> email;?></td>
					<td style="word-wrap: break-word"><?php echo $disp1[$i] -> mobile;?></td>
					<td style="word-wrap: break-word"><?php echo $disp1[$i] -> city;?></td>
					<td style="word-wrap: break-word"><?php echo $disp1[$i] -> postalcode;?></td>
					<td style="word-wrap: break-word"><?php echo $disp1[$i] -> country;?></td>
					<td>
...
<?php	
	...
	{
			global $wpdb;
			$comments = $wpdb->get_var('SELECT note FROM ' . $wpdb->prefix . sm_bookings . ' WHERE id = ' . '"' . $_REQUEST['custoId'] . '"');
			?>
                                                   ...
                                                   if(jQuery("#customeremail1").hasClass('in_error'))
                                                   {
													...
													function updatecustomer()
													{
														...
														function bookbackend(h)
														{
															...
															jQuery('body').append("<div class='box_bookingLink' style='border: 10px solid #525252;left:"+clientWidth+"px;'><div class='close1'>"+titles+"</div><div class='close'>X</div><div id='booking_backend'></div><div id='loading' class='loading'><img src='"+uri+"/images/loading.gif'/></div></div>");
														
															jQuery('#booking_backend').load(uri+"/backendbooking.php?custId="+hid,function(){jQuery('#loading').remove();jQuery('.maincontainer1').css('display', 'block');});
