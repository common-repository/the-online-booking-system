		...
		jQuery.extend(jQuery.fn, {
			miniColors : function(o, data) {
				var create = function(input, o, data) {
					...
					var trigger = jQuery('<a class="miniColors-trigger" style="background-color:  #'+ color +';position:absolute;left:125px;' + color + '" href="#" id="editColor"></a>');
					...
					input.bind('onload.miniColors', function(event) {
						setColorFromInput(input)
					});
				...
				var show = function(input) {
					...
					selector.append('<div class="miniColors-colors" style="background-color: #FFF;"><div class="miniColors-colorPicker"><div class="miniColors-colorPicker-inner"></div></div>').append('<div class="miniColors-hues"><div class="miniColors-huePicker"></div></div>').css({
					...
					jQuery(document).bind('mousedown.miniColors touchstart.miniColors', function(event) {
						...
						if(testSubject.hasClass('miniColors'))
							return;
						hide(input)
					});
					jQuery(document).bind('mouseup.miniColors touchend.miniColors', function(event) {
						event.preventDefault();
						input.data('mousebutton', 0).removeData('moving')
					}).bind('mousemove.miniColors touchmove.miniColors', function(event) {
						event.preventDefault();
						if(input.data('mousebutton') === 1) {
							if(input.data('moving') === 'colors')
								moveColor(input, event);
							if(input.data('moving') === 'hues')
								moveHue(input, event)
						}
					});
					if(input.data('open')) {
				...
				switch(o) {
					case'readonly':
						jQuery(this).each(function() {
							if(!jQuery(this).hasClass('miniColors'))
								return;
					...
					case'value':
						if(data === undefined) {
							if(!jQuery(this).hasClass('miniColors'))
								return;
							var input = jQuery(this), hex = expandHex(input.val());
