 ...
 */
(function($){
	jQuery.fn.highlight = function(element_params){
		...
		var params = jQuery.extend({}, defaults, element_params);

		return this.each(function(){
			var code_container = jQuery(this);
			var code_class = jQuery(code_container).attr('class');
			var code_lang = jQuery(code_container).attr('lang');
			var code_lang_class = '';
			if(code_lang!='') {
				code_lang_class = ' '+code_lang;
			} 
			jQuery(code_container).wrap('<div class="highlight'+code_lang_class+'"></div>');
			var highlight_container = jQuery(code_container).parent();
			...
			switch(code_lang) {
				case 'html':
					code = jQuery.highlightCode.hightlight_html(code);	
					break;
				case 'css':
					code = jQuery.highlightCode.hightlight_css(code);	
					break;
				case 'php':
					code = jQuery.highlightCode.hightlight_php(code);	
				  break;
				default:
					code = jQuery.highlightCode.hightlight(code);	
					break;
			}
			...
			if(params.source==true) {
				...
				jQuery(code_container).after(source);
				jQuery(code_container).before(tabs);
			}
			
			
			//init tabs
			if(params.source==true) {
				var tabs = jQuery(highlight_container).find('ul.tabs li');
				jQuery.each(tabs , function(i,tab){
					jQuery(tab).click(function() {
						jQuery(tabs).removeClass('active');
						var tab_class = jQuery(tab).attr('class');
						jQuery(highlight_container).find('pre[class!="'+tab_class+'"]').css('display', 'none');
						jQuery(highlight_container).find('pre[class^="'+tab_class+'"]').css('display', 'block');
			...
			jQuery(code_container).replaceWith('<pre class="'+code_class+'">'+code+'</pre>');
			
			//zebra
			if(params.zebra==true) {
				jQuery(highlight_container).find('pre[class="'+code_class+'"] '+params.list+' li:even').addClass('even');
			}
		});
	};
	
	jQuery.highlightCode = {
	    ...
	    funcs = new RegExp(get_keywords(funcs), 'gi');
			...
			code = code
				...
				.replace(/\/\/(.*$)/gm,'<span class="com">//$1</span>')
				//replace variables
				.replace(/\jQuery(\w+)/g,'<span class="var">$$$1</span>')
				//replace functions
