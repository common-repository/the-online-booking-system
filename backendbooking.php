...
<link rel='stylesheet' id='popup_style-css'  href='<?php echo $url;?>css/front.css' type='text/css' media='all' />
<link rel='stylesheet' id='popup_style-css'  href='<?php echo $url;?>css/calendar5.css' type='text/css' media='all' />
<div id="divhid"></div>
...
function nextButton03()
{
		if(document.getElementById('hitime'))
		{
			if(document.getElementById('hitime').value != "")
		     {  
					...
					filepath55 = uri + "/servicehours.php";
					jQuery.ajax
					({
						type : "POST",
						...
						success : function(data) 
						{
							...
							if(test == '1')
							{
									...
									jQuery("#step201").css("display","none");
									jQuery("#step301").css("display","none");
									//jQuery("#step401").css("display","block");
									jQuery("#step501").css("display","none");
									var radios = document.getElementsByName('radio3');
									var val;
									var empnm;
		...
		function bookItNow1()
		{
			...
			$emailch = $wpdb->get_var('SELECT email FROM ' . $wpdb->prefix . sm_clients . ' where id=' . '"' . intval($_REQUEST['custId']) . '"'); ?>
